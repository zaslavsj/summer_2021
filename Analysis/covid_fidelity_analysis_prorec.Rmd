...
while (T) {
  ...
  for (i in clust_vect) {
    ...
    for (counter in 1:10) {
      ...
      # weights obtained from RSKC. For the 'fidelity' data frame, choose 
      # columns from 2 to ncol(fidelity) - 1.
...
fidelity_top_features <- fidelity_6_clusts[, names(fidelity_6_clusts) %in% 
                                                rskc_weights_100_runs$Gene_Cell]

# Append the brain region and cluster labels to the 'fidelity_top_features'
# data frame.
fidelity_features_clusts <- bind_cols(fidelity_6_clusts[, c(1,66)],
...
fidelity_features_clusts2 <- fidelity_features_clusts %>% 
  ...
  add_column(fidelity_features_clusts2)
```
...
fidelity_clust_bubble_mean <- fidelity_features_clusts2 %>% 
  group_by(cluster_labels2) %>% 
  summarize_at(vars(AXL_Astrocyte, SCARB1_Oligodendrocyte, 
                    ADAM17_Oligodendrocyte, NRP1_Astrocyte,
                    SCARB1_Microglia, NRP2_Microglia,
                    AXL_Oligodendrocyte, CTSL_Oligodendrocyte), mean) %>%
...
gene_level_order <- c("AXL_Astrocyte", "SCARB1_Oligodendrocyte", 
                      "ADAM17_Oligodendrocyte", "NRP1_Astrocyte",
                      "SCARB1_Microglia", "NRP2_Microglia", 
                      "AXL_Oligodendrocyte", "CTSL_Oligodendrocyte")
...
region_clust_order <- c("DI", "HIP", "AMY", "MID", "MED",
                        "CB", "FCX", 
                        "PON", 
                        "STR", "LIM",
                        "OCX", "TCX", "PCX",
...
# # Alternate version of bubble plot using ggballoonplot. Note: the y-axis does 
# # not display the correct ordering of the features (i.e. it is in reverse
# # order, from highest to lowest weight).
# bubble_region2 <- ggballoonplot(fidelity_region_bubble_mean, aes(x = Brain.Region, 
#                                                                  y = Gene_Cell),
#                                 size = 5,
#                                 fill = "value") +
#   scale_fill_viridis(direction = -1) +
#   labs(fill = "Mean Fidelity Score") +
#   theme(
...
dendro <- ggdendrogram(rskc_dendro)
dendro
```
