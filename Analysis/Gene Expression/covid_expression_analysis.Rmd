...
```{r include=FALSE}
# Export 'all_expression' data frame as a .csv file
write.csv(all_expression, "/Users/rachelkwan/Documents/Lab/summer_2021/Data/COVID Data - 29 Genes - 1281 Samples from 16 Regions - No Transformation.csv", row.names = FALSE)
```

# Build Data Frame

The first thing we have to do is clean our data into the preferred $N \times p$ format for clustering, then we can impute the data using the 5 nearest neighbors.

```{r}
# Build the data frame of regions vs gene expressions for each sample
region.reformatted.log2.exp.data = all_expression %>%
  # Convert the data frame to long format
  melt(id.vars = c("Sample", "Region", "Years"), variable.name = "Gene", value.name = "Expression") %>%
  # Separate the Sample column into 3 strings: the sample ID, the region, and the hemisphere
  separate(col = Sample, into = c("Sample", "Region2", "Hemisphere"), sep = "_") %>%
  # Combine the gene name, sample ID and hemisphere to make a new column called GeneSample
  unite("GeneSample", c(Gene, Sample, Hemisphere)) %>%
  # Pivot the data so each GeneSample gets its own column (for clustering)
  dcast(Region ~ GeneSample, value.var = "Expression") %>%
  # Move the Regions column to the rownames
  column_to_rownames("Region")  %>%
  # Select only columns with 4 or less NAs
  select_if(~sum(is.na(.)) <= 4)

# Impute the missing data
imputed.region.clustering.results.log2 = impute.knn(data = as.matrix(region.reformatted.log2.exp.data), k = 5, 
                                               colmax = 0.25, rng.seed = 262626)

# Convert the results into our new data frame for clustering
myExpression = data.frame(imputed.region.clustering.results.log2$data)

# Store the region names in a vector called regions
...
ggplot(rskc.cluster.regions.long, aes(x = Region_1, y = Region_2, fill = Matches)) +
  geom_tile() +
...
### PDF of LOESS Curves for Z-scored for 16 Regions by Genes
```{r}
