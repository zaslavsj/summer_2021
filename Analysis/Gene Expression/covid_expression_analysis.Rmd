...
# Select desired columns from original full data frame, and turn 'Sample' column
# into row names.
...
# computing the average expression of each gene within a given brain region. Then
# filter to keep only desired regions.
gene_expression <- all_expression %>% 
  group_by(Region) %>%
  summarize_at(vars(all_of(genes)), mean) %>%
  rename(Brain.Region = Region) %>%
  filter(Brain.Region %in% regions_16)
