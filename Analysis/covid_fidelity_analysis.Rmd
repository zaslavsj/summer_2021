...
gene_level_order <- c("NRP1_Astrocyte", "NRP1_Neuron", "DPP4_Astrocyte", 
                      "NRP1_Microglia", "ITGB3_Oligodendrocyte", 
                      "NRP1_Oligodendrocyte", "ITGB3_Neuron", "ITGB3_Microglia")
...
>>>>>>> 4aa0e16cfd74ffac0b16a62463772496670d7115
...
region_clust_order <- c("STR", "AMY", "MED", "PON", 
                        "PCX", "FCX", "TCX", "OCX", "CB",
                        "WM", "DI",
                        "IN", "LIM", "BF", "CLA", "HIP", "GP", "MID")
...
# Compute the mean fidelity score for each gene_celltype for each cluster label.
# Pivot data frame for further plotting steps.
fidelity_clust_bubble_mean2 <- fidelity_features_clusts %>% 
  group_by(cluster_labels) %>% 
  summarize_at(vars(NRP1_Astrocyte, NRP1_Neuron, DPP4_Astrocyte,
                    NRP1_Microglia, ITGB3_Oligodendrocyte, NRP1_Oligodendrocyte,
                    ITGB3_Neuron, ITGB3_Microglia), mean) %>% 
...
# Make data continuous, for plotting 
fidelity_clust_bubble_mean2$`1`=as.numeric(levels(fidelity_clust_bubble_mean2$`1`))[fidelity_clust_bubble_mean2$`1`]
fidelity_clust_bubble_mean2$`2`=as.numeric(levels(fidelity_clust_bubble_mean2$`2`))[fidelity_clust_bubble_mean2$`2`]
fidelity_clust_bubble_mean2$`3`=as.numeric(levels(fidelity_clust_bubble_mean2$`3`))[fidelity_clust_bubble_mean2$`3`]
fidelity_clust_bubble_mean2$`4`=as.numeric(levels(fidelity_clust_bubble_mean2$`4`))[fidelity_clust_bubble_mean2$`4`]

ggballoonplot(fidelity_clust_bubble_mean2,
              fill = "value",
              size = 8) + 
  scale_color_gradient2(low="blue", mid="white",
                        high="red", space ="Lab", guide = "colourbar",
