...
while (T) {
  ...
  for (i in clust_vect) {
    ...
    for (counter in 1:10) {
      ...
      # When i = 4 and counter = 1 (i.e. on the first run of RSKC using 4 clusters),
      # assign 'fidelity' to a new dataframe 'fidelity_4_clusts' to save the cluster 
      # labels for this run 
      if ((i == 4) & (counter == 1)){
        ...
        # to smallest, extract the names of the genes in this order,
        # convert this object into a data frame, and store this info in
        # an object 'weights_4_clusts' in a column called 'gene'.
        weights_4_clusts <- sort(rskc_results_list[[1]]$weights, 
                                 decreasing = T) %>% 
          names() %>% 
          as.data.frame() %>% 
          rename('gene' = ".")
        ...
        # Assign the ordered weights for the current item in 'rskc_list' into
        # 'weights_4_clusts', in a column called 'weight', 
        weights_4_clusts$weight <- sort(rskc_results_list[[1]]$weights,
                                        decreasing = T) %>%
          unname() 
    
        # Impose a factor order on the contents of 'weights_4_clusts$gene' in 
        # the current order. 
        weights_4_clusts$gene <- factor(weight_df$gene,
                                        weight_df$gene)
      }
...
```{r}
fidelity_4_clusts <- fidelity_4_clusts %>%
  select
```
