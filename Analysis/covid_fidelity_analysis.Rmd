...
# Save png of bubble plot
bubble_clust_path <- file.path(here("Plots", 
                                    paste0('RKSC_Bubbleplot_byCluster.png')))
...
bubble_region_path <- file.path(here("Plots", 
                                     paste0('RKSC_Bubbleplot_byRegion.png')))
...
<<<<<<< HEAD
dendro_plot <- plot(rskc_dendro %>% set("branches_lwd", 4), labels = FALSE)
=======
...
```{r}
init_densne <- function(data = weighted_fidelity, workdir = getwd(), no_dims = 2, initial_dims, perplexity = 5, theta,
...
run_densne <- function(weighted_fidelity, no_dims, perplexity, theta, randseed, 
    ...
    if(missing(initial_dims)) { 
        initial_dims <- dim(weighted_fidelity)[[2]]
    }
    if(missing(use_pca)) { 
        use_pca <- FALSE
    }
    ...
    if(initial_dims < dim(weighted_fidelity)[2]) { 
        if(use_pca) { 
            pca_out <- prcomp(weighted_fidelity, center=TRUE, scale=TRUE, retx=TRUE)
            data <- pca_out$x[,1:initial_dims]
        }
        else { 
            data <- weighted_fidelity[,1:initial_dims]
        }
    }
