...
while (T) {
  ...
  for (i in clust_vect) {
    ...
    for (counter in 1:10) {
      ...
      # Assign 'fidelity' to new dataframe 'fidelity_4_clusts' and add a new column
      # for cluster labels when i = 4 and counter = 1, i.e. first run of RSKC using
      # 4 clusters
      if ((i == 4) & (counter == 1)){
      fidelity_4_clusts <- fidelity
      fidelity_4_clusts$cluster_labels <- rskc_results_list[[1]]$labels
      }
