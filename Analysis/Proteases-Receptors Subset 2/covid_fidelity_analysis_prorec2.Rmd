...
library(dabestr) # For visualizing estimation statistics
...
regions <- c("FCX", "PCX", "TCX", "OCX", "STR", "DI", "HIP", "AMY", "CB")
...
```{r unclustered heat map, fig.align = 'center', fig.width = 7, fig.height = 5}
...
```{r clustered heat map, fig.align = 'center', fig.width = 7, fig.height = 5}
...
bubble_clust <- ggplot(fidelity_clust_bubble_mean, aes(x = Gene_Cell,
                                                       y = cluster_labels2)) +
  geom_point(aes(fill = Mean_Fidelity), colour = "black", pch = 21, size = 7) +
  coord_flip() +
...
celltype_raincloud <- ggplot(fidelity_celltype, aes(x = cluster_labels,
  ...
  geom_jitter(aes(colour = cluster_labels),
              width = 0.1, size = 1, alpha = 0.5, show.legend = FALSE) +
...
## Visualizing Estimation Statistics

### Cummings Estimation Plot
```{r, fig.align = 'center', fig.width = 7, fig.height = 4}
# Prepare data for generating estimation plots.
estim_plot_data <- dabest(fidelity_celltype, x = cluster_labels, y = Fidelity,
                          idx = list(c(1, 2, 3)),
                          paired = FALSE)

# Calculate mean differences for the estimation data.
cluster_mean_diff <- mean_diff(estim_plot_data)

# Create Cummings estimation plot.
estim_plot <- cluster_mean_diff %>% plot()
estim_plot
```
