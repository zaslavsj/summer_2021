...
heatmap <- heatmap.2(rskc.region.prop, 
                     scale = "none", 
                     col = ylOrRd(100),
                     Rowv = rskc_dendro,
                     Colv = rskc_dendro,
...
# Change number in file name to match cluster number
heatmap_path <- file.path(here("Plots", 
                               paste0('RSKC_heatmap_K=3_expression.png'))) 

png(file = heatmap_path,
    units = "in",
    width = 8,
    height = 4,
    res = 300
...
```{r}
# Run heatmap code for each desired number of clusters and then use this 
# chunk to visualize them all at once
```
