...
genes <- as_vector(colnames(gene_expression_data)[4:31])
...
while (T) {
  ...
  for (i in clust_vect) {
    ...
    for (counter in 1:10) {
      ...
      ###### Apply weights from RSKC to gene_expression ######
      ...
      # weights obtained from RSKC.
      weighted_expression <- sweep(t(gene_expression[,2:(ncol(gene_expression)-1)]), 
                                 MARGIN = 1, weights, `*`) %>%  
        t()
