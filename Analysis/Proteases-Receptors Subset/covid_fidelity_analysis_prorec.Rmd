...
title: |
  ...
  | Proteases and Receptors Subset
...
# Define a vector containing all SARS-CoV-2 related protease and rector genes
# of interest.
genes_prorec_subset <- c("ANPEP", "ENPEP", "ACE2", "DPP4", "SCARB1", "AXL",
  ...
  for (i in clust_vect) {
    ...
    if (i == clust_vect[length(clust_vect)]) {
      ...
      for (n in elbow_clust_vect) {
        ...
        # 'myFidelity', which has brain region as rows and gene_celltype as
        # columns. Assign RSKC's output as an entry in 'elbow_rskc_list'.
    ...
    # Create vector of the weights obtained from RSKC and assign them to
    # 'weights'. Make empty matrix 'weighted_fidelity' for new weighted
    # fidelity scores.
...
while (T) {
  ...
  # Assign an empty list to 'tsne_list_3', 'tsne_list_4', and 'tsne_list_6'.
  tsne_list_3 <- list()
  tsne_list_4 <- list()
  tsne_list_5 <- list()
  tsne_list_6 <- list()
  ...
  # Create empty data frames to store the cluster assignments and cluster
  # weights for each of the 10 runs.
  ...
  for (i in clust_vect) {
    ...
    for (counter in 1:10) {
      ...
      # 'myFidelity', which has brain region as rows and gene_celltype as
      # columns. Assign RSKC's output as an entry in 'rskc_results_list'.
      ...
      # Create new df 'tsne_out' which contains the two dimensions obtained
      # from tSNE and corresponding regions
      tsne_out <- tsne$Y %>%
        data.frame(regions) %>%
        rename(Brain.Region = regions, V1 = X1, V2 = X2) #rename columns
...
region_heatmap1_path <- file.path(
  here("Plots/Proteases-Receptors Subset/RSKC Heat Maps",
       paste0('RSKC_brain_region_clustering_heatmap_K=6_prorec.png'))
  )
...
region_heatmap2_path <- file.path(
  here("Plots/Proteases-Receptors Subset/RSKC Heat Maps", 
       paste0('RSKC_brain_region_clustering_heatmap_dendro_K=6_prorec.png'))
  )
...
bubble_clust_path <- file.path(
  here("Plots", paste0("RSKC_bubbleplot_clusters_K=6_prorec.png"))
  )
...
rskc_dendro2 <- rotate(rskc_dendro, c(11:13, 17, 18, 16, 15, 14, 6:10, 1:5))
...
```{r, echo = FALSE, include = FALSE, eval = FALSE, eval = FALSE}
...
bubble_dendro_path <- file.path(
  here("Plots/Proteases-Receptors Subset/RSKC Bubble Plots",
       paste0("RSKC_brain_region_dendro_K=6_prorec.png"))
  )
...
```{r generate region bubble plot, warning = FALSE, fig.align = 'center', fig.width = 8, fig.height = 5}
...
bubble_region_path <- file.path(
  here("Plots", paste0('RSKC_bubbleplot_regions_K=6_prorec.png'))
  )
...
viol_plot_path <- file.path(
  here("Plots", paste0("celltype_violin_plot_prorec.png"))
  )
...
```{r generate raincloud plot, message = FALSE, fig.align = 'center', fig.width = 12, fig.height = 8}
...
raincloud_plot_path <- file.path(
  here("Plots", paste0("raincloud_plot_prorec.png"))
  )
