...
### RSKC Weights
```{r}
# Compute the standard deviation of the weights across 100 runs for each gene.
weights_sd <- apply(rskc.region.weights[-1], 1, sd) %>% 
  as_tibble() %>% 
  rename(Weight_SD = value)

# Compute the average weight for each gene across 100 RSKC runs. Append the
# average weights to the previously prepared data frame containing the standard
# deviation of the weights and the names of the genes. Rename the weight column.
weights_100_runs <- rowMeans(rskc.region.weights[-1]) %>% 
  as_tibble() %>% 
  cbind(weights_sd, rskc.region.weights[1]) %>% 
  rename(Avg_Weight = value)

# Join data frames
rskc.avg.weights.gene <- inner_join(weights_100_runs, rskc.region.weights,
                                    by = "Case")

# Obtain the top 8 average weights to show in the bar plot
rskc.avg.weights.gene.top <- rskc.avg.weights.gene %>%
  arrange(desc(Avg_Weight)) %>%
  top_n(8)

# Create a bar plot of the RSKC feature weights for the top 8 genes.
weights_barplot <- ggplot(rskc.avg.weights.gene.top, aes(x = reorder(Case, -Avg_Weight), 
                                                         y = Avg_Weight)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = Avg_Weight - Weight_SD, 
                    ymax = Avg_Weight + Weight_SD),
                size = 0.8,
                width = 0.3,
                colour = "black") +
  labs(x = "Gene",
       y = "Average RSKC Feature Weight") +
  theme(
...
# Save the average RSKC weight bar plot as png in appropriate folder destination
# within the project directory.
weights_barplot_path <- file.path(
  here("Plots",
       paste0('RSKC_top_8_weights_K=5_full.png'))
  )

png(file = weights_barplot_path,
    units = "in",
    width = 8,
    height = 6,
