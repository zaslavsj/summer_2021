...
# Define a vector containing the 13 integrins of interest.
genes_integrins_subset <- c("ITGB3", "ITGA5", "ITGB1", "ITGB6", "ITGAV",
                            "ITGB8", "ITGB2", "ITGA2B", "ITGAM", "ITGAL",
                            "ITGA3", "ITGA8", "ITGB5")
...
fidelity_subset <- all_fidelity %>%
  filter(Gene %in% genes_integrins_subset) %>% # Includes five genes
  select(-c(2,3)) 
...
fidelity <- fidelity_subset.wide %>% 
  ...
  slice(match(regions, Brain.Region)) %>% # Reorder rows to match entries in 'regions' vect
...
This chunk serves as a snapshot of the RSKC over 10 runs (i.e. different set.seed values) in order to visualize the variation in cluster label assignments for each observation. The 10 runs were carried out for each of $K = 4, 5, 6, 7$.
...
while (T) {
  ...
  # Assign the values of 4, 5, 6, 7 to 'clust_vect'.
  clust_vect <- c(4, 5, 6, 7)
  ...
  # for each of the 10 runs.
    ...
    rskc_region_labels_7 <- data.frame("Region" = rownames(myFidelity))
  ...
  for (i in clust_vect) {
    ...
    for (counter in 1:10) {
      ...
      # the corresponding 'rskc_region_labels_3', 'rskc_region_labels_4' or 
      # 'rskc_region_labels_5'.
      if (i == 4) {
      ...
      if (i == 7) {
        rskc_region_labels_7[counter + 1] <- rskc_results_list[[counter]]$labels
        colnames(rskc_region_labels_7)[counter + 1] <- paste("Run_", counter, sep = "")
      }
      ...
      # Use if statements to assign 'tsne_scatter' as an entry in 'tsne_list_4',
      # 'tsne_list_5', 'tsne_list_6', or 'tsne_list_7' depending on the current i.
      if (i == 4) {
      ...
      if (i == 7) {
        tsne_list_7[[counter]] <- tsne_scatter
      }
...
```{r tSNE plots (4 clusters), echo = FALSE, fig.align = 'center', fig.height = 14, fig.width = 6}
...
```{r tSNE plots (5 clusters), echo = FALSE, fig.align = 'center', fig.height = 14, fig.width = 6}
...
```{r tSNE plots (6 clusters), echo = FALSE, fig.align = 'center', fig.height = 14, fig.width = 6}
...
```{r tSNE plots (7 clusters), echo = FALSE, fig.align = 'center', fig.height = 14, fig.width = 6}
# Create 5x2 figure for the 10 scatter plots using K = 7 for RSKC.
ggarrange(tsne_list_7[[1]], tsne_list_7[[2]],
          tsne_list_7[[3]], tsne_list_7[[4]],
          tsne_list_7[[5]], tsne_list_7[[6]],
          tsne_list_7[[7]], tsne_list_7[[8]],
          tsne_list_7[[9]], tsne_list_7[[10]],
          ncol = 2,
          nrow = 5)
```
...
Having previously selected the number of clusters to use for RSKC (i.e. 6), this portion of the code aims to evaluate the proportion of brain regions clustered together over 100 runs (i.e. 100 different set.seed values).
...
for (i in 1:length(regions)) {
  for (j in 1:length(regions)) {
    ...
    rskc.cluster.regions.wide[i, j] <- sum(rskc.region.labels.t[[regions[i]]] == 
                                          rskc.region.labels.t[[regions[j]]])
...
region_heatmap1_path <- file.path(
  here("Plots/Integrins Subset/RSKC Heat Maps",
       paste0('RSKC_brain_region_clustering_heatmap_K=6_integrins.png'))
  )
...
region_heatmap2_path <- file.path(
  here("Plots/Integrins Subset/RSKC Heat Maps", 
       paste0('RSKC_brain_region_clustering_heatmap_dendro_K=6_integrins.png'))
  )
...
# Select only the top eight features from the 'fidelity_6_clusts' data frame.
fidelity_top_features <- fidelity_6_clusts[, names(fidelity_6_clusts) %in% 
                                            rskc_weights_top_8$Gene_Cell]

# Append the brain regions and cluster labels from fidelity_6_clusts
# to 'fidelity_top_features' and assign to 'fidelity_features_clusts'.
fidelity_features_clusts <- bind_cols(fidelity_6_clusts[, c(1, ncol(fidelity_6_clusts))],
...
```{r prepare df for bubble plot by clusters}
...
```{r generate cluster bubble plot, fig.align = 'center', fig.width = 8, fig.height = 4}
...
```{r save cluster bubble plot png, echo = FALSE, include = FALSE, eval = FALSE}
# Save the generated cluster bubble plot as png in appropriate folder destination
# within the project directory.
bubble_clust_path <- file.path(
  here("Plots/Integrins Subset/RSKC Bubble Plots", 
       paste0("RSKC_bubbleplot_clusters_K=6_integrins.png"))
  )
...
```{r prepare df for bubble plot by regions}
...
rskc_dendro2 <- rotate(rskc_dendro, c(15, 14, 16:18, 12, 13, 9:11, 6:8, 5, 1:4))
...
# Make a vector that contains the brain regions in order of the clusters.
...
bubble_dendro_path <- file.path(
  here("Plots/Integrins Subset/RSKC Bubble Plots",
       paste0("RSKC_brain_region_dendro_K=6_integrins.png"))
  )
...
```{r generate region bubble plot, warning = FALSE, fig.align = 'center', fig.width = 8, fig.height = 4}
...
```{r save region bubble plot png, echo = FALSE, include = FALSE, eval = FALSE}
...
bubble_region_path <- file.path(
  here("Plots/Integrins Subset/RSKC Bubble Plots",
       paste0("RSKC_bubbleplot_regions_K=6_integrins.png"))
  )
...
# fidelity score.
fidelity_celltype <- pivot_longer(fidelity_6_clusts, cols = -c("Brain.Region",
                                                      "cluster_labels"),
...
```{r save violin plot png, echo = FALSE, include = FALSE, eval = FALSE}
# Save the generated violin plot as png in appropriate folder destination
# within the project directory.
viol_plot_path <- file.path(
  here("Plots/Integrins Subset/Cluster Violin Plots",
       paste0("celltype_violin_plot_integrins.png"))
  )
...
```{r generate raincloud plot, fig.align = 'center', fig.width = 12, fig.height = 8}
...
celltype_raincloud <- ggplot(fidelity_celltype, aes(x = cluster_labels,
  ...
  coord_flip() +
  geom_point(aes(y = Fidelity, color = cluster_labels), 
             position = position_jitter(width = .15), 
             size = 0.5, 
             alpha = 0.8) + 
  geom_boxplot(width = 0.1, guides = FALSE, outlier.shape = NA, alpha = 0.5) + 
  labs(x = "Cluster",
       y = "Fidelity Score",
       fill = "Cluster") +
...
raincloud_plot_path <- file.path(
  here("Plots/Integrins Subset/Cluster Raincloud Plots",
       paste0("raincloud_plot_K=6_integrins.png"))
  )
