---
title: "COVID-19 Project: RSKC Analysis for Fidelity Scores - Integrins Subset"
...
# Define a vector containing the 12 integrins of interest.
genes_integrins_subset <- c("ITGB3",
                            "ITGA5",
                            "ITGB1",
                            "ITGB6",
                            "ITGAV",
...
fidelity_features_clusts <- bind_cols(fidelity_6_clusts[,c(1,50)],
...
```{r save cluster bubble plot png, echo = FALSE, include = FALSE}
# Save the generated cluster bubble plot as png in appropriate folder destination
# within the project directory.
bubble_clust_path <- file.path(here("Plots", 
...
rskc_dendro2 <- rotate(rskc_dendro, c(15:16, 17:18, 12:14, 9:11, 5:8, 1:4))

# Make a vector that contains the brain regions in order of the clustered heatmap.
...
cluster_colours <- c("brown2", "brown2", "darkorange", "darkorange", 
                     "cornflowerblue", "cornflowerblue", "cornflowerblue", 
                     "darkgreen", "darkgreen", "darkgreen", "darkgreen", "coral1", 
                     "coral1", "coral1", "darkmagenta", "darkmagenta", "darkmagenta", 
                     "darkmagenta")
...
# Plot dendrogram of brain regions made previously for heat map.
dendro <- ggdendrogram(rskc_dendro2)
dendro
```

```{r save region bubble plot png, echo = FALSE, include = FALSE}
# Save the generated brain region bubble plot as png in appropriate folder destination
# within the project directory.
bubble_region_path <- file.path(here("Plots", 
...
# fidelity score.
fidelity_celltype <- pivot_longer(fidelity_6_clusts, cols = -c("Brain.Region", "cluster_labels"),
...
## Rain-Cloud Plot for Visualizing Cluster Fidelity Scores
```{r generate raincloud plot, fig.align = 'center', fig.width = 12, fig.height = 8}
# Create raincloud plot showing the fidelity score distributions for each cluster.          
raincloud <- ggplot(data = fidelity_celltype, 
                    aes(y = Fidelity, x = cluster_labels, fill = cluster_labels)) + 
  ...
  labs(x = "Cluster", y = "Fidelity Score", fill = "Cluster") +
...
```{r save raincloud plot png, echo = FALSE, include = FALSE}
# Save the generated raincloud plot as png in appropriate folder destination
# within the project directory.
raincloud_plot_path <- file.path(here("Plots", 
                                      paste0('Raincloudplot_clusters_integrins.png')))
